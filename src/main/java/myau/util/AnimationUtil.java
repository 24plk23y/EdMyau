...
public class AnimationUtil {
    ...
    public static float progress(
        ...
        float elapsed = MathUtil.clampValue(
        ...
        return ease(
                MathUtil.clampValue(progress, 0.0F, 1.0F),
                easing
        );
    }
    ...
    public static float value(
        ...
        return begin
                + progress(startTime, duration, partialTicks, easing)
                * (end - begin);
    }
    ...
    public static double smooth(
        ...
        double factor = 1.0D
                - Math.exp(-(double) deltaMillis / response);

        return current + (target - current) * factor;
    }

    public static float easeOutBack(float progress) {
        float t = MathUtil.clampValue(
                progress,
                0.0F,
                1.0F
        ) - 1.0F;
