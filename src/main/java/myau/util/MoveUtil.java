...
public class MoveUtil {
    ...
    private static int movementLockCount;
    ...
    public static float getDirectionYaw() {
        ...
        return MathHelper.wrapAngleTo180_float(
                (float) Math.toDegrees(
                        Math.atan2(
                                MoveUtil.mc.thePlayer.motionZ,
                                MoveUtil.mc.thePlayer.motionX
                        )
                ) - 90.0f
        );
    }
    ...
    public static int getSpeedLevel() {
        ...
        if (MoveUtil.mc.thePlayer.isPotionActive(Potion.moveSpeed)) {
            speedLevel = MoveUtil.mc.thePlayer
    ...
    public static float getAllowedHorizontalDistance() {
        float slipperiness = MoveUtil.mc.thePlayer.worldObj
                .getBlockState(
                        new BlockPos(
                                MathHelper.floor_double(MoveUtil.mc.thePlayer.posX),
                                MathHelper.floor_double(
                                        MoveUtil.mc.thePlayer.getEntityBoundingBox().minY
                                ) - 1,
    ...
    public static double[] predictMovement() {
        ...
        if (inputMagnitude >= 1.0E-4f) {
            ...
            float sinYaw = MathHelper.sin(
                    MoveUtil.mc.thePlayer.rotationYaw * (float) Math.PI / 180.0f
            );
            float cosYaw = MathHelper.cos(
                    MoveUtil.mc.thePlayer.rotationYaw * (float) Math.PI / 180.0f
            );
            ...
            return new double[]{
                    strafeInput * cosYaw - forwardInput * sinYaw,
                    forwardInput * cosYaw + strafeInput * sinYaw
            };
        }
    ...
    public static void fixStrafe(float targetYaw) {
        if (MoveUtil.isMovementLocked() || MoveUtil.mc.thePlayer == null) {
            return;
        }
