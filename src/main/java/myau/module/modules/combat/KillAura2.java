...
public class KillAura2 extends Module {
    ...
    public final IntProperty angleStep = new IntProperty("angle-step", 90, 10, 180);
    ...
    if (RotationUtil.angleToEntity(entityLivingBase) > this.fov.getValue().floatValue()) {
        return false;
    }

    if (!this.throughWalls.getValue() && RotationUtil.rayTrace(entityLivingBase) != null) {
        return false;
    }
        ...
        if (this.isEnabled() && target != null) {
            if (TeamUtil.isEntityLoaded(target.getEntity())
                ...
                if (this.showTarget.getValue() == 1) {
                    Myau.moduleManager.modules.get(HUD.class);
                    Color color = ((HUD) Myau.moduleManager.getModule(HUD.class)).getColor(System.currentTimeMillis());
