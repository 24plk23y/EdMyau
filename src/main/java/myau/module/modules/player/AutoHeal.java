...
public class AutoHeal extends Module {
    ...
    private boolean dropBowl = false;
    ...
    public final BooleanProperty drop = new BooleanProperty("drop", false);
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (stack != null && stack.hasDisplayName()) {
                ...
                if (stack.getItem() instanceof ItemSoup
                        ...
                        || (name.contains("§a") && name.contains("Assist Soup")))) {
                    return i;
                }
            }
        }
    ...
    private void dropBowlIfNeeded() {
        if (!this.drop.getValue() || !this.dropBowl || this.dropSlot == -1) {
            return;
        }
        ItemStack stack = mc.thePlayer.inventory.getStackInSlot(this.dropSlot);
    ...
    public void onTick(TickEvent event) {
        if (!this.isEnabled()) {
            ...
            switch (event.getType()) {
                case PRE:
                    this.dropBowlIfNeeded();

                    boolean percent = (float) Math.ceil(
                            mc.thePlayer.getHealth() + mc.thePlayer.getAbsorptionAmount()
                    ) / mc.thePlayer.getMaxHealth() <= (float) this.health.getValue() / 100.0F;
                    ...
                    if (this.shouldHeal
                        ...
                        if (slot != -1) {
                            ...
                            ItemStack healingItem = mc.thePlayer.inventory.getStackInSlot(slot);
                            this.dropBowl = this.drop.getValue() && healingItem != null && healingItem.getItem() instanceof ItemSoup;
                            this.dropSlot = this.dropBowl ? slot : -1;
