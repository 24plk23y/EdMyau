...
public class BindCommand extends Command {
    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 3) {
            if (args.size() == 2 &&
                    (args.get(1).equalsIgnoreCase("l")
                            || args.get(1).equalsIgnoreCase("list"))) {

                List<Module> modules = Myau.moduleManager.modules.values()
                        .stream()
                ...
                if (modules.isEmpty()) {
                    ChatUtil.sendFormatted(
                            String.format("%sNo binds&r", Myau.clientName)
                    );
                } else {
                    ChatUtil.sendFormatted(
                            String.format("%sBinds:&r", Myau.clientName)
                    );
    ...
    /**
     * Generate keyboard/mouse key completion candidates.
     */
    private List<String> getKeyCompletions() {

        List<String> result = new ArrayList<>();

        /*
         * Special values accepted by BindCommand.
         */
        result.add("none");
        result.add("null");
        result.add("0");
         ...
         * Keyboard keys.
         *
         * Keyboard.getKeyName() / getKeyIndex() use LWJGL
         * key names.
         */
    ...
    private int getMouseButtonIndex(String buttonName) {
        ...
        if (buttonName.startsWith("MOUSE")) {
            try {
                ...
                if (buttonNum >= 0
                        && buttonNum < Mouse.getButtonCount()) {
        ...
        int buttonIndex =
                Mouse.getButtonIndex(buttonName);
