...
public class GuiAccountManager extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiButton(
        6, width / 2 - 50, height - 76, 95, 20, "Offline"
        ));
    ...
    if (account.isOffline()) {
        ...
        notification = new Notification(
                TextFormatting.translate(
                        String.format("&aOffline login successful! (%s)&r", username)
                ),
                5000L
        );
...
if (StringUtils.isBlank(username)) {
    ...
    boolean current = username.equals(SessionManager.get().getUsername());

    if (account.isOffline()) {
        // 离线 = 黄色
        // 当前 = 黄色 + 粗体
        username = current
                ? String.format("&e&l%s", username)
                : String.format("&e%s", username);
    } else {
        // 正版 = 绿色
        // 当前 = 绿色 + 粗体
        username = current
                ? String.format("&a&l%s", username)
                : String.format("&a%s", username);
    }
}
